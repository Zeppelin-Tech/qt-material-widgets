...
QtMaterialCheckableIcon::QtMaterialCheckableIcon(const QIcon &icon, QtMaterialCheckable *parent)
    : QWidget(parent), m_checkable(parent), m_color(Qt::black), m_icon(icon), m_iconSize(24),
      m_opacity(1.0) {
  Q_ASSERT(parent);
...
QSize QtMaterialCheckableIcon::sizeHint() const { return QSize(m_iconSize, m_iconSize); }
