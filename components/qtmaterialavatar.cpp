 ...
 *  \internal
 */
QtMaterialAvatarPrivate::QtMaterialAvatarPrivate(QtMaterialAvatar *q) : q_ptr(q) {}
...
void QtMaterialAvatar::setSize(int size) {
  ...
  if (!d->image.isNull()) {
    d->pixmap = QPixmap::fromImage(
        d->image.scaled(d->size, d->size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
  }
...
void QtMaterialAvatar::setImage(const QImage &image) {
  ...
  d->pixmap = QPixmap::fromImage(
      image.scaled(d->size, d->size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
  update();
}
...
void QtMaterialAvatar::paintEvent(QPaintEvent *event) {
  ...
  if (!isEnabled()) {
    ...
    painter.drawEllipse(
        QRectF((width() - d->size) / 2, (height() - d->size) / 2, d->size, d->size));
    return;
  }
